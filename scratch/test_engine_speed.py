...
def optimized_run_backtest(
    ...
    # Simple metrics wrapper matching source
...
print("Profiling original run_backtest...")
t0 = time.time()
for _ in range(30):
    res_orig = original_run_backtest(df_dict)
t_orig = time.time() - t0
print(f"Original: 30 iterations took {t_orig:.4f} seconds ({t_orig/30:.4f}s per run)")

print("Profiling optimized run_backtest...")
t0 = time.time()
for _ in range(30):
    res_opt = optimized_run_backtest(df_dict)
t_opt = time.time() - t0
print(f"Optimized: 30 iterations took {t_opt:.4f} seconds ({t_opt/30:.4f}s per run)")
