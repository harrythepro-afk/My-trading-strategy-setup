...
def memory_optimized_run_backtest(
    ...
    sl_level_arr = master_df["sl_level"].tolist() if "sl_level" in master_df.columns else [0.0] * n_candles
    tp_level_arr = master_df["tp_level"].tolist() if "tp_level" in master_df.columns else [0.0] * n_candles
    
    # Pre-allocate equity curve list to avoid dynamic resizing allocations
    equity_curve = [initial_balance] * n_candles
    ...
    for i in range(n_candles):
        ...
        if active_trades:
            ...
            if current_equity <= 0:
                ...
                trade_peaks.clear()
                balance = 0.0
                # Fill remainder of equity curve with 0
                for j in range(i, n_candles):
                    equity_curve[j] = 0.0
                break
